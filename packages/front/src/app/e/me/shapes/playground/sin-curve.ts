...
const defaultParams = {
  ...
  /**
   * Phase offset of the sine wave (in periods).
   */
  phase: 0,

  /**
   * Offset to apply to the "x" coordinate (linear axis). 
   */
  offset: 0,
...
export class SinCurveOld extends Curve<Vector3> {
  ...
  override getPoint(t: number, optionalTarget?: Vector3): Vector3 {
    ...
    const tx = t * l + o
    const ty = a * Math.sin((f * t + o) * PI2)
    const tz = (za ?? a * .1) * Math.cos((f * t + o) * PI2)
