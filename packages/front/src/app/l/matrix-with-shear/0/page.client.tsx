...
import { LineGeometryUtils } from 'some-utils-three/geometries/line'
...
class TwinObject extends Group {
...
function MyScene() {
  ...
  useGroup('my-scene', function* (group, three) {
    ...
    setup(twin1.objA, { x: 1, y: 1 })
    setup(twin1.objB, { x: -1, y: 1, rotation: ['0deg', '180deg', '0deg'] })

    const twin2 = setup(new TwinObject(), group)
    setup(twin2.objA, { x: 1, y: -1 })
    setup(twin2.objB, { x: -1, y: -1, rotation: ['90deg', '90deg', '0deg'] })
